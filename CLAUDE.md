...
.github/             The automatic checks on every pull request (see "Automatic checks"): workflows/checks.yml (lint, types, build), workflows/migration-check.yml, and scripts/check-migrations.sh, the migration rule itself.
...
- Tests are not part of the Vercel build and do not run in the automatic checks yet (see below).

## Automatic checks

GitHub runs two checks on every pull request to `main`, from `.github/workflows/`. Neither uses a secret, and neither can reach any database.

- **"Lint, types and build"** (`checks.yml`): `npm ci`, `npx prisma generate`, `npx next typegen`, `npm run lint`, `npx tsc --noEmit`, `npm run build`. The build needs no environment values at all. If a later change makes it need one, give the workflow a clearly fake value, never a real key (rule 7).
- **"Production migration"** (`migration-check.yml`, with the rule in `.github/scripts/check-migrations.sh`): if the pull request adds, changes or deletes anything under `prisma/migrations/`, it fails until the pull request has the label `production-migrated`. Evan adds that label after migrating production (rule 3, step 4); adding or removing a label runs the check again. A pull request with no migration passes on its own. **The label is a person's word, not proof**: GitHub cannot see the production database and must never be given its connection string. If another migration is pushed after the label was added, take the label off, migrate, and put it back. A change to `schema.prisma` with no migration file does not trip this check.
- **`npm test` does not run here.** The tests need the Neon `testing` branch, and its connection string is not stored in GitHub. Adding it later is Evan's decision and needs three things: the testing branch's pooled string saved as a GitHub Actions secret called `TEST_DATABASE_URL`; a second secret for the guard in `vitest.setup.ts` to compare against, holding production's real host name with no password in it (the guard compares only the endpoint, and a made-up host would let it pass without checking anything); and a workflow step that runs `npm test` with both. The guard still applies there and is never loosened, and a pull request from a fork never receives the secrets.
- **Whether a check is required before merging** is set by Evan in the GitHub ruleset on `main`, not in code. Claude never changes the ruleset.
