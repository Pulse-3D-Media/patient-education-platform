...
**We are in Phase 2, the clinic dashboard:** logins (done, see Auth), clinics and people (done, see Roles), the Pulse 3D master dashboard (done, see the four surfaces), the clinic admin area's sections (done: overview, shared links, people, billing), clinic branding (implemented, device acceptance pending), paused links a clinic can turn back on (done, see "How a link expires, pauses, and is turned back on"), then billing and real video hosting.
...
The only functions that take no `clinicId` are the ones behind the patient's link, where the share code in the URL is the key: `getShareByCode()`, `recordSharePlay()` and `requestShareRenewal()` (the patient's one tap on a paused link, which writes one timestamp and nothing else). That is the whole list. If a task seems to need another public-by-code function, say so first.
...
No names, no dates of birth, no email addresses, no medical record numbers, no clinical notes. A share link is tied to a **procedure**, a **clinic** and the **surgeon** it is from, never to a patient. No patient profiles, no marketing or advertising tracking on the patient page, and no patient-facing sales features, ever.

The staff side is different: a staff member's identity is used for sign-in, for permissions, for the clinic log and, for a surgeon, as the name on the links they send ("Sent by Dr. Jane Smith"), and that is fine. It is patient data that never enters the system.
...
| `app/library` | **The surgeon**, in the room | Tablet or phone | Clerk sign-in, any member of an open clinic; Send only for someone holding a seat |
...
**`app/watch` is the patient's page, and it stays about one thing.** No account, no email, no app to install, no consent screen, no second confirmation, no cookie banner before the video plays. One procedure, the clinic's name (and logo), calm plain language, large controls, and nothing else: no navigation, no related videos, no sales, no link to anywhere but play. See the speed rule below. The one exception to "nothing but play": a link that has paused shows one button, Ask my clinic, and no box to type into (see "How a link expires, pauses, and is turned back on").

**`app/library` is the exam-room surface.** A surgeon opens it mid-consult, finds the procedure, and either plays it right there on their own device or sends the patient a link. It is used standing up, in front of a patient, under time pressure. **It obeys the same speed rule as the patient viewer**: tablet-first, big touch targets, **browse to playing in two taps**, Play and Send as **separate one-tap actions**, no dense tables. It shows only what the clinic's plan allows (see Category entitlements): a category off the plan is a locked tile and a locked page, never a list of videos. Everyone in the clinic can browse and play; **only someone holding a seat gets Send**, and a link they send is from them automatically, with no picker (the page hides the button from everyone else, and the action checks the seat again).
...
| Page | Job |
|---|---|
| `/admin` | The office manager's **overview**: the notice from Pulse, what needs a look (links that stopped working, links about to expire), **Links waiting to be reactivated** (paused links a patient has asked about, each with a button to `/admin/reactivate/<code>`, the page that turns one back on after a Confirm press; see "How a link expires, pauses, and is turned back on"), a few counts, the newest five links, and a card into each section. Not the full list of anything: it reads five counts, five rows and at most ten waiting links (`summarizeSharesForClinic()`, `listRecentSharesForClinic()` and `listRenewalRequestsForClinic()` in `lib/db/shares.ts`), never the whole history. |
| `/admin/links` | **Making a link.** Category pills and a search box, and one row per video this clinic may share with one button, **Create link**, in the brand colour (never red). Pressing it opens a small box on that row, "Which doctor is this link from?", with a dropdown of the people holding a seat and **nothing picked in advance** (decided by Evan on 2026-09-25, after trying one picker at the top of the page: the doctor is chosen for every link). Choosing a doctor makes one new link and turns the box into its menu: Copy link, Download QR code, Print QR code. Closing the menu does not delete the link. **There is no list of past links, on purpose** (Prompt 4 of the 2026-09-21 build plan): the app does not know which patient got which link, so the office never needs to find one again, and there is no cancel button. The pamphlet (`/admin/print/<code>`) and the QR picture (`/admin/qr/<code>`) belong to it. |
| `/admin/people` | Everyone in the clinic and the clinic's seats: "2 of 3 seats in use", Member / Member with admin on each person, Give a seat and Remove, the name patients see on each seated person's links ("Dr. Jane Smith" by default) with Change, inviting someone (an invitation holds a seat) and revoking an invitation, the account owner's handoff, and in plain words anything that needs settling (people waiting for a seat, a plan that pays for fewer seats than are taken, no account owner). "Choose a plan first" until the clinic is open. Clerk's organization panel stays at the bottom for the clinic's name and logo. See "Roles" and "Surgeon seats". |
...
- **Unpublishing a video is the one control for withdrawing content, and it stops every link to it, old ones included**: the patient page shows the calm "not available right now" page, views are not counted, `createShare()` refuses new links, and the overview's newest links say "Not working". Publishing it again makes them all work again. Nothing else withdraws a video that has already been sent.
- **Patient links already issued outlive the clinic-side changes around them.** Taking a category off a clinic's plan, pausing or closing the clinic, running out of seats, or a category no longer being for sale: none of these cancels a link a patient already has. An issued link follows its own rules only: its expiry, or the video being unpublished. (Clinics could cancel a link until the list of past links was removed; the clinic side no longer offers it. `deleteShareForClinic()` in `lib/db/shares.ts` stays, unused, for whenever a way to withdraw one link is decided.) (The patient page does not check the clinic's status, on purpose.) This policy changes only when a prompt says so in as many words, and any page copy that suggests otherwise is wrong and should be fixed to match.
...
| Email | **Resend**, over plain HTTPS, no package | Only `lib/email.ts` talks to it, with the fetch built into Node. Without `RESEND_API_KEY` and `EMAIL_FROM` nothing is sent and the app runs as before. One email exists: a patient asking for a paused link back. See "How a link expires, pauses, and is turned back on". |
...
|---|---|---|
| `org:admin` | Member with admin | `/admin` and everything under it: making share links (from any seated surgeon), QR codes and pamphlets, `/admin/people` (invite, revoke, admin on or off, remove, give a seat, the name patients see), branding and billing. Plus everything a member can do. |
| `org:member` | Member | `/library`, and sending links to patients from it when they hold a seat. |
...
**The seat** is described under "Surgeon seats". A seat grants no admin permission and a role never takes a seat away, **with one exception, decided in Prompt 4 of the 2026-09-21 build plan: only someone holding a seat sends links to patients**, because every link is from a surgeon. Whoever holds a seat may send from the library, and an admin may make a link from any seated person on `/admin/links`; nobody without a seat (the account owner who took none included) is either.
...
/// transaction that holds the clinic's row lock. Two ids and, optionally,
/// the name patients see on the person's links. No email, no role.
model SeatAllocation {
  ...
  createdAt   DateTime      @default(now())
  updatedAt   DateTime      @updatedAt
  displayName String?                             // how this person's name appears to patients on links they send ("Jane Smith, PA-C"); empty means "Dr. First Last" from Clerk. Set by an admin on /admin/people
...
model AppSettings {
  ...
  maxRenewals   Int      @default(3)    // times a clinic may turn a paused patient link back on (0 to 10; 0 means never). Read live, so a change applies to every link at once
  updatedAt     DateTime @updatedAt
}
...
model Share {
  ...
  renewalRequestedAt DateTime?                    // when the patient last tapped "Ask my clinic"; one request per link per day. Cleared when the link is turned back on
  lastRenewedAt      DateTime?                    // when the clinic last turned it back on
...
- **Seat and owner changes are in the clinic log**, written in the same transaction as the change where the change is ours: a seat given or let go, an invitation sent or revoked, a removal (under "<name> (clinic admin)"), a handoff, an owner set by Pulse staff, and anything `checkSeats()` put right (under "seats"). Admin on or off is logged right after Clerk confirms it. A hold whose invitation was never made is not written up. Nothing about the person is stored but a Clerk id and, when an admin has typed one, the name patients see on their links (`SeatAllocation.displayName`, staff data): other names appear only in the log's sentences, and invited email addresses live only in Clerk.
- **Who a link is from.** Every new link records its surgeon (`Share.senderUserId`) and the name patients see (`Share.senderName`), copied when it is made, so a later name change or the surgeon leaving never changes a link already sent. The name is the one typed for them on `/admin/people` (`setPatientName()` in `lib/seat-changes.ts`, checked by `parseDisplayName()` in `lib/sender-name.ts`, logged, under the clinic's lock), else "Dr. First Last" from Clerk, else none, in which case the link names only the clinic; their email is never used. `lib/senders.ts` lists who can be picked and checks a pick with Clerk (`resolveSender()`); `createShare()` then reads the seat with a share lock (`lockSenderSeat()` in `lib/db/access.ts`) in the transaction that writes the link and throws `SenderRefusedError`, writing nothing, when that person holds no seat at this clinic right now. The patient page and the pamphlet say "Sent by Dr. Jane Smith, <clinic>"; a link made before this (or by the tests and seed scripts, which pass no sender) says "From <clinic>" as before (`sentByLine()`).
...
**Placeholder videos.** A video with `isPlaceholder` true carries a real procedure name but plays a sample animation, so the library can be tested before the finished animations exist. This is not the same as unpublished: placeholders are visible on purpose. The app marks them everywhere they appear: an amber "Placeholder" mark on the library card, in the player, across the top of the patient page, and in the admin lists. The Send panel carries the selected video's placeholder mark, and so does the printed pamphlet (in black and white, so it survives any printer). **If you show a video somewhere new, carry the mark with it.** Placeholders are added, edited and replaced on `/pulse/videos` like any other video; `prisma/seed-placeholders.ts` only fills a fresh database, and `prisma/seed-video.ts` (the first real animation) never touches them.
...
**How a link expires, pauses, and is turned back on.** The rule is `lib/expiry.ts`, pure, and every page and write asks it. A link made by `createShare()` is a **first-play link** (`Share.expiryPolicy` FIRST_PLAY): it stops after the platform's `unclaimedDays` (90 by default; Evan sets it to 365 on `/pulse/settings`) if nobody plays it, and the first real play (`recordSharePlay()`, called by the patient player) moves `expiresAt` to that moment plus the days copied onto the link when it was made (`Share.daysAfterFirstPlay`: the clinic's `viewDaysOverride` when Pulse staff have set one, else the platform's `viewDays`, 7 by default; Evan sets it to 10). That move happens exactly once: the claim is one UPDATE whose WHERE says "still unclaimed, still before the deadline, video still published", so two first plays at once move the deadline once, and later plays only add to the count. A play at the deadline itself or after it, on a cancelled link, or on an unpublished video writes nothing, whenever the page was opened: the server's clock decides. Every link made before this rule, and any link written without the column, is a **legacy link** (FIXED, the database default): its date was set when it was made and playing it changes nothing but the count. No backfill touched them, and none ever should. A settings edit changes links made from then on only; the numbers a link carries are the ones it was promised with, and a save that lands while a link is being made waits for it (the settings lock, above). **Both numbers have limits: a whole number of days from 1 to 365** (`MIN_LINK_DAYS` and `MAX_LINK_DAYS` in `lib/expiry.ts`, the limit the clinic override always had). The settings form and the override form refuse anything outside them, and `resolveShareTerms()` refuses again right before the numbers become dates, with a `ShareTermsError` whose message is a plain sentence and nothing written; `/admin/links` then says links cannot be made and asks for Pulse 3D instead of failing. A stored days-after-first-play outside the limits (nothing in the app writes one) never becomes a deadline either: `canClaimFirstPlay()` treats it as a fixed link. The overview's newest links say all of this in words (`shareExpiryState()`), and `/admin/links` and the library's Send panel read the same resolved numbers (`getShareTerms()`) that `createShare()` copies onto a new link, so the words and the link cannot disagree. Nothing on the patient page mentions a deadline; a play that could not be recorded is simply not counted, and a first-play link then keeps its longer unclaimed deadline, so the patient is never worse off for it.

**When those days run out, a played link is not over: it has PAUSED** (the build plan's word is "dormant"; decided by Evan and Van on 2026-09-21, reversing the old rule that there was no renewal). Nothing is ever deleted. The rule is `renewalState()` in `lib/expiry.ts`: a first-play link that has been played and has run out is **paused** while renewals are left, and **finished** otherwise; `lib/db/shares.ts`, the patient page and the admin page all ask it, so they cannot disagree.

- **The patient side.** A paused link shows a calm page with one button, Ask my clinic (`AskClinic.tsx`), and **no box to type into**: a patient would type personal details into any box that was there (rule 2). The tap sends the link's code and nothing else; `requestShareRenewal()` (public, by code, the third such function) writes one timestamp, `Share.renewalRequestedAt`, **at most once per link per day** (`RENEWAL_REQUEST_GAP_MS`), in one UPDATE whose WHERE says "still paused, and not asked about within the last day", so two taps at once record one request and the clinic is told once. The page then says "Your clinic has been asked. Try this same link again in a day or so", and shows that instead of the button on any visit within the day. No deadline, no count, nothing technical.
- **The clinic side.** A request emails every office admin of the clinic (`lib/renewal-email.ts` through `lib/email.ts`): the procedure, the surgeon the link is from, when it was made, how many renewals are left, and one button, Reactivate this link. Nothing about the patient, because nothing exists, and the email says so. The button opens `/admin/reactivate/<code>`: a page that needs a sign-in, is admins-only, looks the code up within the signed-in admin's own clinic (another clinic's code, or a forged one, is not-found), shows the link's details and where it stands, and has a Confirm button that is a POST (`reactivateLinkAction`). **Opening the email, or the page, changes nothing**: mail programs and hospital filters open links to scan them, and a forwarded email must not be able to turn anything on. Only Confirm does. The address in the email is built from a trusted origin (`pickTrustedOrigin()`), never from the Host header as it came.
- **Without email** (no `RESEND_API_KEY` and `EMAIL_FROM`, the service down, no admins, no trusted origin), the request is still recorded, and the overview lists it under **Links waiting to be reactivated** (`listRenewalRequestsForClinic()`, bounded, most recently asked first) with a button to the same page. That list is also where an admin finds a request whose email they missed. The request is written before any email is attempted, and a failed email is one word in the server log, never the code.
- **Turning it back on** is `renewShareForClinic(clinicId, code, actorName)`: one transaction that holds the clinic row FOR SHARE (the clinic must be open: a paused, cancelled or out-of-grace clinic is refused, and the page says why), the link's row FOR NO KEY UPDATE (`lockShareForRenewal()` in `lib/db/access.ts`) and the settings lock shared, asks the rule, and only for a **paused** link with a published video moves `expiresAt` to now plus the link's own `daysAfterFirstPlay`, adds one to `Share.renewalsUsed`, sets `lastRenewedAt`, clears the request (so the link is not listed as waiting again the moment it next pauses), and writes one clinic-log entry under "<name> (clinic admin)", all together. **Two admins, or a double click, renew once**: the second waits for the first and is told the link is already working. A working link, a finished one, a legacy link, one nobody played, one whose video is not published, a code that is not this clinic's, and a clinic that is not open are all refused with a plain sentence and nothing written. Plays count again from then on; the first play is history and never moves the deadline again.
- **How many times: `AppSettings.maxRenewals`**, on `/pulse/settings`, default 3, from 0 to 10 (`MIN_RENEWALS`, `MAX_RENEWALS`; 0 means no link can be turned back on). **Unlike the day counts it is not copied onto the link: it is read at the moment it is needed, so a change applies to every link at once, links already sent included** (decided by Evan on 2026-09-25). A stored value outside the limits means zero, never unlimited. When the renewals are used up and the link runs out again it is **finished**: the calm expired page, telling the patient to ask the practice for a new link, with no button.
- **What is never paused.** A **legacy link** (FIXED) behaves exactly as before: its date was final, and playing it changes nothing but the count. A first-play link **nobody ever played** that ran out is finished, not paused (decided by Evan on 2026-09-25): it was never opened, so there is nothing to give back. Every first-play link made before this shipped can pause and be renewed like a new one, for the days it was issued with (7 for most of them); no row was changed for it (decided the same day). A paused link whose video is unpublished shows the calm expired page and cannot be turned back on until the video is back, like every link to an unpublished video.
- **Email setup**, done by Evan, not by code: a Resend account, the sending domain verified there (the DNS records Resend lists, added at the domain's registrar, so the emails are not treated as spam), then `RESEND_API_KEY` and `EMAIL_FROM` in `.env` and in Vercel. Until then requests are recorded and listed, and nobody is emailed.
...
```
app/watch/[code]     The patient viewer. Phone-first, no login, no navigation. AskClinic.tsx is the one button on a paused link (and its confirmation); CallButton.tsx the tap-to-call button; actions.ts holds recordPlay and requestReactivation.
...
app/admin/links/     Shared links, where a link is made: LinkRows.tsx (the pills and search, one row per video with Create link, the doctor dropdown it opens, and the small menu for the new link) and actions.ts (createLinkAction). No list of past links.
app/admin/print/     The printable pamphlet for one share link: who sent it, and the placeholder mark when there is one.
app/admin/qr/        The QR code image for one share link.
app/admin/people/    The People section: the clinic's seats in use, everyone in the clinic with Member / Member with admin, their seat, Give a seat and Remove; Invite someone and the open invitations with Revoke; the owner's handoff; and Clerk's panel for the clinic's name and logo. PeopleControls.tsx holds the buttons and forms (PatientNameEditor is the name patients see), ConfirmButton.tsx the "Are you sure?" popup. actions.ts holds the Server Actions, each of which goes through lib/seat-changes.ts. Admins only, and nothing to manage until the clinic is open.
...
app/admin/reactivate/ Turn a paused link back on: /admin/reactivate/<code>, where the email's button and the overview's waiting list land. page.tsx shows the link and where it stands (and why for a clinic that is not open); ReactivateForm.tsx is the Confirm form; actions.ts holds reactivateLinkAction, the only thing that turns a link back on. Admins of open clinics only; another clinic's code is not-found.
...
lib/trusted-origin.ts pickTrustedOrigin(): which address Stripe may send a person back to, and which address goes in an email. The Host header only picks among addresses the deployment knows are its own. Pure.
lib/email.ts         Sending email, server only: the only file that talks to Resend, over plain HTTPS with the fetch built into Node, no package. emailIsConfigured() and sendEmail(), which never throws and logs only the kind of failure, never the key, an address or the message.
lib/renewal-email.ts The email a clinic's office admins get when a patient asks for a paused link back: renewalRequestEmail() (the words, pure, with the one button to /admin/reactivate/<code>) and notifyClinicOfRenewalRequest() (every admin of the clinic's organization, through lib/email.ts, under an idempotency key). Server only.
...
lib/sender-name.ts   Who a link is from, in words, pure and safe for the browser: defaultSenderName() ("Dr. First Last"), parseDisplayName() (the check on a typed name), effectiveSenderName(), sentByLine() ("Sent by Dr. Jane Smith, <clinic>" or "From <clinic>"), and the Sender type.
lib/senders.ts       Who a link can be from, server only: listSenders() (everyone holding a seat, for the doctor dropdown) and resolveSender() (a pick checked with Clerk, turned into what createShare() writes).
...
lib/expiry.ts        The expiry rule, pure: MIN_LINK_DAYS and MAX_LINK_DAYS (1 to 365) with isValidLinkDays(), resolveShareTerms() (the settings and a clinic's override become the numbers a new link carries, refused with a ShareTermsError outside the limits), isExpired(), canClaimFirstPlay(), expiryAfterFirstPlay(), shareExpiryState() (expired, awaiting, played, fixed) and daysLeftText(). And the pausing rule: MIN_RENEWALS and MAX_RENEWALS (0 to 10) with isValidRenewalCount(), renewalState() (working, paused, finished and why), expiryAfterRenewal(), canRequestRenewal() with RENEWAL_REQUEST_GAP_MS (one request per link per day), and finishedLinkMessage(). No database; lib/db/shares.ts, the patient page, the Pulse forms and the admin pages ask it.
...
- Who a link is from is tested at every layer. `lib/sender-name.test.ts` is the name rules with plain values, and `lib/senders.test.ts` the doctor dropdown's list (only people holding a seat, still in the clinic, with what patients will see) and the server's check of a choice. `lib/db/shares.test.ts` checks `createShare()` with a sender: the id and name copied, the typed name preferred, the name kept after a later change or the seat going, no name made up, and someone with no seat here (a seat at another clinic included) or a malformed id refused with nothing written. `lib/db/shares.sender.race.test.ts` is the forced overlap: the seat is deleted, or its name changed, after `createShare()` has read it and before the insert, and the change has to wait; its controls with a plain read show it getting through, and the app's own writers (`releaseSeat()`, `setSeatDisplayName()`) wait too. `app/admin/links/actions.test.ts` (the Clerk stand-in) covers a member, a closed clinic, signed out, the pick being someone without a seat, someone from another clinic, nothing or junk, and a Clerk outage; `app/library/[category]/actions.test.ts` covers Send from the signed-in surgeon and a refusal for someone without a seat. The name on People is in `lib/seat-changes.test.ts` and `app/admin/people/actions.test.ts`; the page renders (`app/admin/pages.test.tsx`, `app/library/pages.test.tsx`, `app/watch/[code]/page.test.tsx`) prove Shared links draws no picker up front, the Send button only for them, and "Sent by" on the patient page and the pamphlet, with the old wording for older links.
...
- Pausing and reactivation are tested at every layer. `lib/expiry.test.ts` is the rule with plain values (working, paused, finished and each reason, the maximum read live, the request gap, every sentence). `lib/db/shares.renewal.test.ts` is the database: a played link plays on day 9 and has paused on day 11; the request written once per day, five taps at once writing one, and a request changing one timestamp and nothing else; the clinic turning a link back on for its own days, the maximum number of times and then finished; a working, legacy, never-played and unpublished link, another clinic's code, a forged code, and a paused or out-of-grace clinic refused with nothing written; a legacy link untouched; and the waiting list holding one clinic's asked-about paused links only and dropping a link the moment it is turned back on. `lib/db/shares.renewal.race.test.ts` is the forced overlap: a second reactivation started after the first has read the link and before it writes has to wait and is told the link is already working, and its control with a plain read shows both getting through and the link counted twice, which is what the row lock prevents. `lib/email.test.ts` and `lib/renewal-email.test.ts` are the email with the service and Clerk's people as stand-ins (the words, the one button, names made harmless, every admin and no member, no send without both settings or a trusted origin, refusals answered and never thrown, the key never logged). `app/watch/[code]/page.test.tsx` renders the paused page (one button, no box of any kind, the confirmation within a day of a request, and no button on a finished, legacy, never-played or taken-down link); `app/watch/[code]/actions.test.ts` covers the request written before the clinic is told, the trusted origin, the second tap, junk, and a server failure; `app/admin/reactivate/[code]/actions.test.ts` (the Clerk stand-in) covers signed out, a member, an admin of a clinic that has not paid, another clinic's admin, a forged code, a real press, a second press, and a failure answered in plain words; `app/admin/pages.test.tsx` renders the waiting list on the overview and the reactivate page for an admin, a member, another clinic's admin and a closed clinic. A real email in an inbox, and the button in it, are the browser check with Resend's settings in place.
...
- **The expired state is a real design job, not an error page.** Calm, plain language, tells them to ask their doctor for a new link. Never technical, never red, never the words "error", "invalid" or "403". The same goes for a video that will not load: a plain sentence and a way to try again, never the exception. **The paused state is the same job with one button:** "This link has paused. Tap below to ask your clinic to turn it back on", then "Your clinic has been asked. Try this same link again in a day or so." No box to type into, ever, and no deadline or count on the page.
