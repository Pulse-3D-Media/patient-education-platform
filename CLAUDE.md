...
| `/admin/links` | **Making a link.** "Links are from" at the top (the surgeon, picked from the people holding a seat; the signed-in admin when they hold one), category pills and a search box, and one row per video this clinic may share with one button, **Create link**, in the brand colour (never red). Pressing it makes one new link and opens a small menu for that link: Copy link, Download QR code, Print QR code. Closing the menu does not delete the link. **There is no list of past links, on purpose** (Prompt 4 of the 2026-09-21 build plan): the app does not know which patient got which link, so the office never needs to find one again, and there is no cancel button. The pamphlet (`/admin/print/<code>`) and the QR picture (`/admin/qr/<code>`) belong to it. |
...
app/admin/links/     Shared links, where a link is made: LinkRows.tsx ("Links are from", the pills and search, one row per video with Create link and the small menu for the new link) and actions.ts (createLinkAction). No list of past links.
...
lib/senders.ts       Who a link can be from, server only: listSenders() (everyone holding a seat, for the picker) and resolveSender() (a pick checked with Clerk, turned into what createShare() writes).
...
- Who a link is from is tested at every layer. `lib/sender-name.test.ts` is the name rules with plain values. `lib/db/shares.test.ts` checks `createShare()` with a sender: the id and name copied, the typed name preferred, the name kept after a later change or the seat going, no name made up, and someone with no seat here (a seat at another clinic included) or a malformed id refused with nothing written. `lib/db/shares.sender.race.test.ts` is the forced overlap: the seat is deleted, or its name changed, after `createShare()` has read it and before the insert, and the change has to wait; its controls with a plain read show it getting through, and the app's own writers (`releaseSeat()`, `setSeatDisplayName()`) wait too. `app/admin/links/actions.test.ts` (the Clerk stand-in) covers a member, a closed clinic, signed out, the pick being someone without a seat, someone from another clinic, nothing or junk, and a Clerk outage; `app/library/[category]/actions.test.ts` covers Send from the signed-in surgeon and a refusal for someone without a seat. The name on People is in `lib/seat-changes.test.ts` and `app/admin/people/actions.test.ts`; the page renders (`app/admin/pages.test.tsx`, `app/library/pages.test.tsx`, `app/watch/[code]/page.test.tsx`) prove the picker lists only seated people, the Send button only for them, and "Sent by" on the patient page and the pamphlet, with the old wording for older links.
