...
- hands the account over on `/admin/people` to someone who already has admin on, after a popup (`handOffOwner()`). The free spot moves with it: the new owner keeps a seat if they had one, the old owner stays in the clinic and now needs a seat like everyone else (waiting for one if none is free). The handoff is checked again under the clinic's row lock, so two tabs handing over at once end with one owner and one log entry.
