...
**Billing is being built in steps, in Stripe test mode only.** Done: the payment-state foundation (what is stored about a clinic's subscription, the rule that turns it into access, and the webhook that keeps it true; see "Billing state"). Also done: self-serve checkout, where an eligible clinic's admin picks a plan on `/admin/billing` and pays on Stripe's own page (see "Self-serve checkout"). Also done: surgeon seat limits (see "Surgeon seats"). Not built: plan changes, updating a card, invoices. **Nothing real is ever charged: the app refuses a live Stripe key, and test-mode checkout is never offered on the production deployment.**
...
| `/admin/people` | People and roles, and who holds the clinic's surgeon seats: "2 of 3 in use", a Surgeon / Staff control on each person, and in plain words anything that needs settling (surgeons waiting for a seat, a change that did not finish saving, a plan that pays for fewer seats than are held). See "Surgeon seats". |
...
**Kind is what a person is for billing:** `surgeon` (a seat the clinic pays for) or `staff` (free). It lives on the Clerk membership's public metadata as `{ kind: "surgeon" | "staff" }`, is asked once at `/onboarding/kind` (an answered question cannot be sent again), and can be changed by an admin on the People page. **Kind never grants a permission.** An admin can be a surgeon; a member can be staff. Whether a surgeon actually **holds one of the clinic's paid seats** is a separate fact, kept in our own table; see "Surgeon seats". Every change of kind goes through `changeKind()` in `lib/seat-changes.ts`, which holds it to the seat limit.
...
The helpers live in `lib/roles.ts` (roles, kind, `isClinicAdmin()`), `lib/people.ts` (the people in a clinic, read from Clerk; it talks to Clerk, never to `lib/db`) and `lib/seat-changes.ts` (the only way a kind is changed).
...
Eleven models and ten enums. If a task seems to need a twelfth model, stop and ask.
...
model Clinic {
  ...
  // Who holds one of the surgeon seats above. See "Surgeon seats" below.
  seatAllocations SeatAllocation[]
}

/// Whether Clerk's copy of a seat agrees with ours yet.
enum SeatSyncState {
  PENDING // reserved here; the "surgeon" label is not confirmed on the person's Clerk membership yet (on its way, or it failed)
  SYNCED  // the person's Clerk membership says surgeon too
}

/// One surgeon seat, held by one person at one clinic. THIS TABLE IS THE
...
model SeatAllocation {
  ...
  reservedAt  DateTime      @default(now())       // when it was last reserved or tried again. A PENDING seat older than a few minutes that Clerk never confirmed is let go
  createdAt   DateTime      @default(now())
  updatedAt   DateTime      @updatedAt
...
A clinic can have as many surgeons **holding a seat** as its plan pays for (`Clinic.surgeonSeats`), and no more. Staff are free and never take one. The rules are `lib/seats.ts` (pure); read the top of that file first.

**Two facts, two homes, and one authority for each.** The **label** ("surgeon" or "staff") is what a person said they are, or what their admin marked them; it lives on the Clerk membership and Clerk is the authority on it. The **seat** is whether that person holds one of the paid seats; it lives in our `SeatAllocation` table, and **that table is the authority**: the number of rows a clinic has there is what is compared with the seats on its plan. Neither grants a permission (see Roles). A person labelled surgeon with no row is a surgeon **waiting for a seat**, and uses the library exactly as before.

- **A seat is only given under the clinic's row lock.** `reserveSeat()` in `lib/db/seats.ts` locks the clinic, counts the seats in use, and only then writes (`readSeatsLocked()` in `lib/db/seat-lock.ts` is the lock-then-count). Two people reaching for the last seat run one after the other and the second is refused. Lowering a plan takes the same lock, so it and a seat being given never cross. The table allows one row per person per clinic, which is what makes a repeated request harmless.
- **A database transaction cannot make a call to Clerk part of itself, and nothing pretends it can.** No lock is ever held across a call to Clerk (it would make a surgeon's Send button wait on another company's server). `changeKind()` in `lib/seat-changes.ts` orders each change so that every way it can stop halfway leaves something harmless. **Becoming a surgeon:** reserve a seat here (PENDING), write the label to Clerk, mark the seat SYNCED. If the Clerk write fails the reservation is let go; if even that fails, or the server dies, a PENDING seat is left, the People page shows "Not saved yet" with Try again, and it lets itself go after `PENDING_WINDOW_MS` (two minutes). **Becoming staff:** write the label to Clerk, then let the seat go; a seat left behind with a staff label is let go by the next check.
- **`changeKind()` is the only way a kind is changed, and `writeKindToClerk()` (in `lib/people.ts`) may be called from `lib/seat-changes.ts` only.** A test reads the source tree and fails if anything else uses it, or calls Clerk's metadata write directly. Every route comes through it: the surgeon question at `/onboarding/kind` and the Surgeon / Staff control (and its Try again) on `/admin/people`. It looks the clinic's own organization up from the clinic's row, and checks with Clerk that the person is a member of **that** organization before anything is reserved or written, so a user id sent by the browser can never reach into another clinic. An invitation cannot produce a seat either: a new member arrives with no kind and is asked the question, and one that arrives already labelled (an invitation made with metadata in Clerk's dashboard) is a surgeon waiting for a seat until the check below gives them one.
- **The surgeon question is asked once.** Whether it has been answered is read from Clerk on the server, in the action and again inside `changeKind()`; an answer that is already there is never changed from `/onboarding`, whatever is sent. Changing it afterwards is the admin's job.
- **A "Yes" with no seat free is kept as a request, and that is the whole of the pre-payment rule.** Before a clinic has paid it has no seats, so the first surgeon's answer cannot take one. It is recorded (the label is written, so they are not asked again), they are let in, and they are a surgeon waiting for a seat: no row, no charge. The same thing happens to a person's own first answer at a clinic that is full. **It is not a way round the limit once the clinic has paid**, because there is no special mode to stay in: waiting surgeons are given seats only up to what the plan pays for, oldest member first, and everyone else keeps waiting and is shown to the admin. An **admin** marking someone a surgeon at a full clinic is simply refused, with a sentence saying what to do, and nothing is written.
- **`checkSeats(clinicId)` brings the table back into line with Clerk** (`planSeatCheck()` decides, `applySeatCheck()` does it under the lock and counts again before each seat it gives). It lets go the seat of someone who has left or is no longer labelled surgeon, confirms a reservation Clerk has since confirmed, and gives free seats to waiting surgeons. It never relabels anyone, never gives more seats than the plan has, and writes nothing (and logs nothing) when there is nothing to put right. An empty answer from Clerk is treated as "could not say", never as "everyone left". There are no background jobs and no Clerk webhooks, so it runs where it matters: when `/admin/people` or a clinic's `/pulse` page is opened, and **before anyone is told a clinic is full**. This is what notices a member removed in Clerk's own panel, a label changed in Clerk's dashboard, and every surgeon marked before seats existed.
- **Seats cannot be lowered below the number in use without saying so.** `checkSeatReduction()` is asked, under the lock, wherever seats can go down: `acceptPlanForCheckout()` and `recordAcceptedPlan()` refuse a plan with fewer seats than are held (this is "when it is scheduled"); `setClinicPlan()` refuses too unless `allowFewerSeatsThanInUse` is set, which the `/pulse` Plan form sends only when the staff member ticked the box that says so (`npm run db:set-plan` has `--allow-fewer-seats`). When a plan **takes effect** short of what is in use (people can be seated between accepting a plan and paying for it), it is applied, because it was paid for, and `reconcileSubscription()` says so in the log. **When changing a paid plan is built, a seat reduction must ask `checkSeatReduction()` both when it is scheduled and again when it is applied.**
- **An over-the-plan clinic is shown, never quietly fixed.** Nobody is relabelled, no seat is taken away and no charge is changed. The gap is said in plain words on `/admin/people`, `/admin/billing` and the clinic's `/pulse` page, the log entry says by how much, and `hasFreeSeat()` refuses every new seat until a surgeon is marked as staff or the seats are raised.
- **Seat changes are in the clinic log**, written in the same transaction as the change: a seat given or let go by an admin (under "<name> (clinic admin)"), a person's own answer, and anything `checkSeats()` put right (under "seats"). A reservation that never completed is not written up.
- **What this does not do.** Kind is still what a person says they are: nothing stops a clinic marking a surgeon as staff to avoid a seat, because kind grants no permission. And a removal in Clerk's panel frees the seat the next time someone looks, not the instant it happens.
...
- **The server prices it (rule 8).** The form says what was picked and nothing about money that is believed: `readPlanSelection()` reads categories, seats, the interval, and which pricing version and total the admin was *looking at*. A clinic id, an amount, a discount or a founding flag added to the request is never read. The price is worked out from the **active** saved version (an estimate is refused) and the categories for sale right now (`listSellableCategories()`), with `practiceType: "clinic"` only after eligibility has been checked. A category that is not for sale, or is coming soon, is not a purchase option; a plan a clinic already has is never reduced because sale eligibility changed later.
...
app/admin/people/    The People section: the clinic's surgeon seats in use, everyone in the clinic with a Surgeon / Staff control (KindControl.tsx, which also shows Waiting for a seat and Try again), and Clerk's invite and role panel. actions.ts is the one Server Action, which goes through changeKind(). Admins only.
app/admin/billing/   Billing: the plan and its estimate, the plan picker, and the subscription once there is one. billing.ts works out the plan on file and its estimate; checkout-view.ts works out what is offered (picker, question, contact Pulse, managed, subscribed, not open); page.tsx turns both into words. Open to an admin of a closed clinic. actions.ts holds the practice question (PracticeTypeQuestion.tsx, asked once), startCheckoutAction (PlanPicker.tsx) and checkPaymentAction. return/ is where Stripe sends the admin back to: page.tsx reads the billing record and grants nothing; Confirming.tsx looks again a bounded number of times and offers Check again.
...
app/onboarding       Set up your clinic (Clerk's CreateOrganization), then the surgeon-or-staff question at /onboarding/kind. actions.ts records the person's own answer, once, through changeKind().
...
lib/people.ts        The people in a clinic, from Clerk: listPeople(), getMember() (is this user id really in this clinic), getSignedInName(). writeKindToClerk() is the label write, which only lib/seat-changes.ts may call. Never touches lib/db.
lib/seats.ts         The surgeon seat rules, pure and safe for the browser: seatSummary() and hasFreeSeat(), checkSeatReduction(), planSeatCheck() (what to change to bring the seats into line with the clinic's people), seatStateOf() (held, waiting, pending, none) and the sentences an admin is shown. Read its top comment first. No database, no Clerk.
lib/seat-changes.ts  The only way a person becomes a surgeon or staff, server only: changeKind() (reserve a seat, write the label to Clerk, confirm; and what happens when any step fails) and checkSeats() (bring a clinic's seats into line with its people, and return everyone with where they stand).
lib/testing/         fake-clerk.ts, an in-memory stand-in for Clerk that the tests swap in. Nothing in the app imports it.
...
- Surgeon seats are tested four ways. `lib/seats.test.ts` is the rules with plain values. `lib/db/seats.test.ts` is the table against the database: seats given until the plan is full, a repeat not counted twice, five people at once for the last seat, one clinic's seats nothing to do with another's, and every guard in `applySeatCheck()`. `lib/db/seats.race.test.ts` is the forced overlap: it wraps `readSeatsLocked()` for one call so a second request is started after the first has counted and before it writes, and checks that the second waited and was refused; its control does the same with a plain count and shows BOTH taking the one seat, which is what the lock prevents and proves the test would catch its removal; a third test lowers the plan in that gap. `lib/seat-changes.test.ts` runs `changeKind()` and `checkSeats()` against the database with the in-memory stand-in for Clerk (`lib/testing/fake-clerk.ts`), which can be made to fail a write, lose a member or have a label changed behind the app's back: a failed Clerk write after the reservation, a failed release on top of that and the pending seat letting itself go, Try again, duplicates, two admins at once, a removed member, an invitation that arrives labelled, a label changed in Clerk's dashboard, surgeons marked before seats existed, the pre-payment request, the replayed onboarding answer, a person in another clinic, and an over-the-plan clinic. It also reads the source tree and fails if anything but `lib/seat-changes.ts` writes a kind. `app/onboarding/actions.test.ts` and `app/admin/people/actions.test.ts` cover signed out, a member, an admin, an admin of another clinic, a closed clinic, and the replay. **A stand-in is not Clerk:** the walkthrough with a real Clerk test organization is a separate check.
