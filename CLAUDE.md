...
lib/expiry.ts        The expiry rule, pure: resolveShareTerms() (the settings and a clinic's override become the numbers a new link carries), isExpired(), canClaimFirstPlay(), expiryAfterFirstPlay(), shareExpiryState() (expired, awaiting, played, fixed) and daysLeftText(). No database; lib/db/shares.ts and the admin pages ask it.
