...
Both players use the browser's own video controls and play inline. The patient player always did, and shows them from the first play on (before it, the big Play button is the only thing on the picture). The library player drew its own controls until this change and now uses the browser's, with no Download entry in its menu; its title, placeholder chip and clinic mark sit in rows above the picture. One measured limit in Chrome and Edge: while keyboard focus is inside the browser's controls, the page never hears Escape; Close always works and Tab reaches it. Media failures show a calm retry state, and retries retain the last playback position. The patient page records only the first actual playing event per load; retries do not count a second play start. Expired, withdrawn and failed-video states offer a tap-to-call number only when the stored phone is valid. The patient enlargement button requests container fullscreen when the browser reports support; denial or absence uses a modal overlay with a close control, Escape, focus restoration, background isolation, scroll restoration, safe-area padding and visual-viewport updates. The library already opens in such an expanded overlay. Browser-native video fullscreen and Picture-in-Picture may omit all HTML branding. These features are branding, never copy protection.
...
lib/branding.ts      The branding rules, pure and safe for the browser: the font list, parseBrandColor(), parseBrandFont(), parseLogoUrl() (https only), and staffTheme() / patientTheme(), which turn one brand colour into shades that stay readable. The Pulse look when nothing is set.
...
app/brand-look.ts    patientLook() and staffLook(): a clinic's stored branding turned into the CSS variables, font class, checked logo address and call link a page draws with.
