...
import { BRAND_FONTS, parseLogoUrl, patientTheme, readBranding, staffTheme, themeVars, type BrandFontKey } from "@/lib/branding";
...
/** The four stored values a look is made from. Any of them may be missing. */
type StoredBranding = {
  logoUrl?: string | null;
  phone?: string | null;
...
function lookFrom(stored: StoredBranding | null | undefined, surface: "patient" | "staff"): Look {
  ...
  const theme = surface === "patient" ? patientTheme(branding.color) : staffTheme(branding.color);
...
/**
 * The look of the staff screens (/library and /admin, near-black) for one
 * clinic. Takes the clinic as lib/clinic.ts returns it, whose colour and
 * font are already checked.
 */
export function staffLook(clinic: { logoUrl: string | null; phone: string | null; branding: { color: string | null; font: BrandFontKey } } | null | undefined): Look {
  return lookFrom(
    clinic ? { logoUrl: clinic.logoUrl, phone: clinic.phone, brandColor: clinic.branding.color, brandFont: clinic.branding.font } : null,
    "staff",
  );
}
