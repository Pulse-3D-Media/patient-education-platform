 ...
 * and its logo (or name) in the banner.
 *
...
export function ClinicShell({ clinic, showAdmin = false, children }: { clinic: CurrentClinic; showAdmin?: boolean; children: ReactNode }) {
  ...
  return (
    <AppShell showAdmin={showAdmin} brand={{ style: look.style, fontClass: look.fontClass, clinicName: clinic.name, logoUrl: look.logoUrl }}>
      {children}
    </AppShell>
  );
}
